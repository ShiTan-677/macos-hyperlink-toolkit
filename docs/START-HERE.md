...
5. 首次出现自动化权限提示时，核对它请求控制的是当前浏览器，然后允许。授权后如果没有复制成功，再从服务菜单运行一次。
...
1. 按同样方法安装其余两个工作流。
2. 在浏览器前台运行 **HLT - Copy Excel Hyperlink**，剪贴板得到 `=HYPERLINK("URL","Title")` 文本。
3. 切到 Microsoft Excel，打开测试工作簿，单击一个空白单元格并退出编辑状态。
4. 运行 **HLT - Paste Excel Formula**。它将公式直接写入当前活动单元格，无需再用 Command + V。
...
5. Allow the expected Automation request to the current browser. Run the service again if the first attempt only completed authorization.
...
Install the other two workflows in the same way. Run **HLT - Copy Excel Hyperlink** with the browser in front. In Microsoft Excel, select an empty cell in a test workbook, leave edit mode, then run **HLT - Paste Excel Formula**. No ordinary paste is needed.
